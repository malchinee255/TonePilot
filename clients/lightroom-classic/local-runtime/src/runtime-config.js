...
const SUPPORTED_PROVIDERS = new Set(['rule', 'openai', 'qwen2'])
...
function providerRuntimeConfig(config, providerValue) {
  ...
  if (provider === 'rule') {
    return { provider: 'rule', apiKey: '', baseUrl: '', model: '' }
  }
...
function normalizeProvider(value) {
  const provider = String(value || 'rule').toLowerCase()
  return SUPPORTED_PROVIDERS.has(provider) ? provider : 'rule'
}
