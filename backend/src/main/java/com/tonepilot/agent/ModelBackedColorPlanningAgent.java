    ...
    public ColorAdjustment plan(Long photoId, String targetStyle, PhotoAnalysis analysis, List<RagSearchItem> knowledgeItems) {
        ...
        try {
            String prompt = """
                    ...
                    照片分析：%s
                    RAG 知识：%s
                    只输出 JSON，字段必须为 style, reason, basic, hsl, effects, steps。
                    参数必须保守，exposure 建议限制在 -1.5 到 1.5 内，其他 Lightroom 滑块限制在 -100 到 100 内。
            ...
            return new ColorAdjustment(
                    ...
                    output.basic() == null ? fallback.plan(photoId, targetStyle, analysis, knowledgeItems).basic() : output.basic(),
                    output.hsl() == null ? fallback.plan(photoId, targetStyle, analysis, knowledgeItems).hsl() : output.hsl(),
