...
flowchart LR
  ...
  D --> E["Agent 控制台"]
  E --> F["用户输入调色意图"]
  F --> G["主 Agent 自主观察、判断和选择下一步"]
  G --> H["OpenAI / Qwen / 管理端知识检索或模型代理"]
...
- 维护当前照片的多轮交互上下文。
- 由主 Agent 自主分析用户意图、照片类型、当前参数和可用工具能力。
- 根据主 Agent 判断决定回复分析、追问用户、检索知识库、规划局部蒙版或调用 Lightroom 全局调色工具。
- 使用用户配置的大模型或管理端模型代理生成本轮调色参数。
- 校验参数范围，只输出本轮需要修改的 Develop Settings。
- 写入 Lightroom 文件桥接任务，并等待插件应用结果。
- 返回主 Agent 的可见判断结果、参数变化、应用结果和预览地址。
- 可选上报管理端事件和 Trace。
...
- `agent/`：主 Agent 模型调用、可见判断结构和运行时编排。
- `admin/`：管理端事件上报客户端。
- `api/`：本地 HTTP API 和 Agent 控制台入口。

用户端不展示固定的后端步骤列表。页面只在请求中展示“思考中...”，请求完成后展示主 Agent 返回的动态判断，包括观察到的线索、判断依据、下一步动作、准备使用的工具动作和可选回复。Trace 和底层步骤用于开发与管理端可观测性，不直接包装成用户端的固定流程。
...
管理端后端保留可控状态机式编排，用于知识库写入、样片分析、评估和服务端流程，而不是开放自治 Agent。这里可以参考 Ragent 的分层方式，将素材导入和 RAG 拆成 `ingestion`、`knowledge`、`rag`、`vector`、`trace`、`evaluation` 等边界。
...
当前管理端已提供基础 RAG 检索接口：待审核知识通过后会生成 Chunk 和本地向量索引，`POST /api/rag/retrieve` 返回 Runtime 可引用的知识片段。后续可继续参考 Ragent 的多通道设计，将检索扩展为风格意图检索、照片元数据检索、用户历史偏好检索、全局向量兜底检索，再通过去重、Rerank、参数安全过滤和 Lightroom 能力过滤形成更稳定的知识上下文。
