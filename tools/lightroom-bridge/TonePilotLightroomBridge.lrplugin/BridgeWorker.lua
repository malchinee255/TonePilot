...
function writeSelectedPhotoState()
    ...
    catalog:withReadAccessDo(function()
        ...
        payload = {
            ...
            metadataDebug = metadataDiagnostics(selected),
...
function exportPreview(photo, previewPath)
    ...
    for _, rendition in exportSession:renditions({ stopIfCanceled = true }) do
        local success, exportedPath = rendition:waitForRender()
        if not success then
            error(exportedPath)
        end
        LrFileUtils.copy(exportedPath, previewPath)
        return
    end

    error("Lightroom 没有生成导出文件")
end
