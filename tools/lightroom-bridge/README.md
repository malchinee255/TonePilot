# TonePilot Lightroom Bridge

这个目录是 TonePilot 的 Lightroom Classic 用户端工程，包含本地 Bridge 服务和 Lightroom Classic Lua 插件。

## 边界

Bridge 只负责本地电脑上的插件协作：
...
Bridge 不再承担旧的后端渲染兼容接口，也不需要摄影师在浏览器上传照片。
...
Lightroom Classic SDK 不能把第三方 Web 面板真正停靠到原生左右侧栏中，因此当前采用“插件菜单入口 + 本地 Bridge 控制台”的方式实现用户端。
...
```text
server.js                              本地 Bridge HTTP 服务和控制台页面
bridge-paths.js                        Windows/WSL 路径适配
start-bridge-wsl.sh                    WSL 启动脚本
start-bridge.ps1                       Windows 启动脚本
install-plugin.ps1                     安装 Lightroom 插件
TonePilotLightroomBridge.lrplugin/     Lightroom Classic Lua 插件
```

运行时日志文件会被 `.gitignore` 忽略。
